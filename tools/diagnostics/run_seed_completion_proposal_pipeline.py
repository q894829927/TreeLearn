"""Run one forest and intercept TreeLearn pointwise arrays for Q5b0."""
...
from tree_learn.util.seed_completion_proposals import (
    analyze_seed_completion_proposal_oracle,
)
...
def run(config_path):
    ...
    output_dir = Path(str(config.seed_completion_proposal_output_dir))
    ...
    def intercept_savez(file, *args, **kwargs):
        ...
        if missing:
            raise ValueError(
                f'Intercepted Q5b0 payload misses {sorted(missing)}.')
        settings = config.seed_completion_proposal
        ...
        if float(grouping.upper_anchor_weight) != 0.0 or bool(getattr(
            ...
            raise ValueError('Q5b0 requires locked base-only 2D grouping.')
        if bool(getattr(grouping, 'use_seed_confidence_filter', False)):
            raise ValueError(
                'Q5b0 requires seed-confidence filtering to be disabled.')
        print(
            f'Q5b0 analyzing {source_plot}: '
            f'{len(settings.expected_target_tree_ids)} fixed Q3 targets.',
            flush=True)
        result = analyze_seed_completion_proposal_oracle(
            ...
            target_tree_ids=list(settings.expected_target_tree_ids),
        ...
        proposal_rows = result.pop('proposal_rows')
        result.update({'source_plot': source_plot, 'split': 'validation'})
        output_dir.mkdir(parents=True, exist_ok=True)
        write_csv(output_dir / 'proposals.csv', proposal_rows)
        (output_dir / 'summary.json').write_text(
            json.dumps(result, indent=2, ensure_ascii=False),
            encoding='utf-8')
        completed['value'] = True
        print(
            f'Q5b0 saved for {source_plot}: '
            f'{result["num_proposals"]:,} GT-free proposals, '
            f'{result["proposal_covered_target_trees"]}/'
            f'{result["num_target_trees"]} targets covered.',
            flush=True)
    ...
    if not completed['value']:
        raise RuntimeError(
            'TreeLearn finished without producing the intercepted Q5b0 '
            'payload.')


def main():
    parser = argparse.ArgumentParser(
        description='Run one fixed-validation Q5b0 proposal Oracle.')
